...
# Latitude-dependent weighting of pelagic carbonate accumulation.
# See latitude_factor.py for the physical justification and references.
from latitude_factor import latitude_factor
...
def write_grid_file_from_xyz(
    ...
    # For example "nearneighbor thickness.xy -R-180/180/-90/90 -I1 -N4 -S1d -rg -Gthickness.nc".
    #
    # Registration: '-rg' forces *gridline* registration explicitly. Our input
    # point grid has data points on the grid lines, and the whole downstream
    # workflow (plotting, movie/video generation) assumes gridline registration.
    # Although gridline registration is the GMT default, we set '-rg' explicitly
    # so that:
    #   (1) the intent is documented in the source rather than depending on a
    #       default that could change in a future GMT release; and
    #   (2) any tool that subsequently reads the grid header sees an
    #       unambiguous registration flag, avoiding the "Longitude range too
    #       small; geographic boundary condition changed to natural" warning
    #       that grdimage emits when a global grid's data span does not equal
    #       360 degrees (a classic symptom of accidental pixel registration).
    # Do NOT change this to pixel registration ('-rp') without also updating
    # every downstream script that ingests these grids.
    gmt_command_line = [
        "gmt",
        "nearneighbor",
        xyz_filename,
        ...
        "-rg",  # Force gridline registration (see comment above).
        "-G{0}".format(grid_filename)]
...
def calc_carbonate_decompacted_sediment_thickness(
    ...
    for point in points:
        ...
        input_points_in_age_grid_reference_frame.append(
            input_lat_lon_point_in_age_grid_reference_frame.to_lat_lon()[::-1])  # swap (lat, lon)
        
        input_lat_lon_point_in_bathymetry_grid_reference_frame = (
            rotation_to_bathymetry_grid_reference_frame * rotation_from_carbonate_grid_reference_frame * point)
        input_points_in_bathymetry_grid_reference_frame.append(
            input_lat_lon_point_in_bathymetry_grid_reference_frame.to_lat_lon()[::-1])  # swap (lat, lon)
    ...
    # Get the ages and bathymetries at the input point locations (in their respective reference frames).
    ages = gmt_grdtrack(input_points_in_age_grid_reference_frame, age_grid_filename)
    bathymetries = gmt_grdtrack(input_points_in_bathymetry_grid_reference_frame, bathymetry_filename)
    ...
    while reconstruction_time <= bathymetry_filename_oldest_time:
        ...
        reconstructed_bathymetrys = gmt_grdtrack(reconstructed_points_in_bathymetry_grid_reference_frame, reconstructed_bathymetry_filename)
        ...
        for index, reconstructed_point_index in enumerate(reconstructed_point_indices):
            ...
            if (not math.isnan(bathymetry_at_reconstruction_time) and
                ...
                # Latitude- and time-dependent reduction of pelagic carbonate
                # accumulation. The depth-only formulation above has no temperature /
                # productivity / saturation-state control, so without this factor the
                # model deposits spurious thick carbonate on young, shallow
                # high-latitude crust (most visibly the Arctic). See latitude_factor.py
                # for the physical justification and references.
                #
                # We pass BOTH the paleo-latitude (so high-latitude crust which
                # originated at low latitude still gets credit for carbonate
                # accumulated before drifting poleward) AND the paleo-time
                # (reconstruction_time) so the correction is automatically switched
                # off in the pre-34 Ma greenhouse, ramped in across the
                # Eocene-Oligocene Transition + Oligocene CO2 drawdown
                # (34 -> 23 Ma; Hoenisch et al. 2023), and fully applied at
                # <= 23 Ma. The carbonate reference frame (anchor plate 701) is the
                # mantle/absolute frame of the Muller 2019 model, so
                # reconstructed_points[index].to_lat_lon() gives the true
                # paleo-latitude.
                paleo_lat, _ = reconstructed_points[index].to_lat_lon()
                lat_weight = latitude_factor(paleo_lat, reconstruction_time)
