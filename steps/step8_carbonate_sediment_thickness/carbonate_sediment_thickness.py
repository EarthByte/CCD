...
def write_data(
    ...
    # The '.xy' XYZ file is only an intermediate for GMT gridding; the '.nc' grid
    # is the product used downstream, so delete the '.xy' once the grid exists.
    # (Guard on the grid existing so a failed GMT run doesn't discard the data.)
    try:
    ...
    while reconstruction_time <= bathymetry_filename_oldest_time:
        ...
        for index, reconstructed_point_index in enumerate(reconstructed_point_indices):
            ...
            if (not math.isnan(bathymetry_at_reconstruction_time) and
                ...
                # / productivity / saturation-state control, so without this factor
                # the model deposits spurious thick carbonate on young, shallow
                # high-latitude crust (most visibly the Arctic). See
                # latitude_factor.py for the physical justification and references.
                #
                ...
                # (reconstruction_time) so the correction is automatically
                # switched off in the pre-34 Ma greenhouse, ramped in across the
                # Eocene-Oligocene Transition + Oligocene CO2 drawdown
                # (34 -> 23 Ma; Hoenisch et al. 2023), and fully applied at
                # <= 23 Ma. The carbonate reference frame (anchor plate 701) is
                # the mantle/absolute frame of the Muller 2019 model, so
